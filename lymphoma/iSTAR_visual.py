...
def extract_cluster_colors(n_clusters, cmap_func):
    cluster_indices = np.arange(n_clusters)
    colors = [cmap_func(i) for i in cluster_indices]
    return colors

def plot_labels(
        labels, filename, cmap_func=None, white_background=True,
        transparent_background=False, interlace=False, reverse=False):
    ...
    if cmap_func is None:
        if n_labels <= 70:
            cmap_func = cmap_tab70
        else:
            cmap_func = plt.get_cmap('turbo')

    # Extract cluster colors
    colors = extract_cluster_colors(n_labels, cmap_func)
    ...
    cbar = plt.colorbar(plt.cm.ScalarMappable(cmap=cmap), ax=ax, orientation='vertical', fraction=0.02, pad=0.02)
    ...
    # Calculate tick positions for centering labels
    tick_positions = np.linspace(0, 1, n_labels + 1)[:-1] + (1 / (2 * n_labels))
    # Remove color bar ticks
    cbar.ax.yaxis.set_ticks_position('none')  # Remove ticks
  # Add custom tick labels to the color bar
    tick_labels = [f'Cluster {i}' for i in range(n_labels)]
    cbar.set_ticks(tick_positions)
    cbar.ax.set_yticklabels(tick_labels)

    # Set font size for color bar labels
    cbar.ax.tick_params(labelsize=8)  # Set label font size
